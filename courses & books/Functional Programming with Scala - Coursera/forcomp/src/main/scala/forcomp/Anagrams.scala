...
object Anagrams {
   ...
   */
  def combinations(occ: Occurrences): List[Occurrences] = occ.foldRight(List[List[(Char, Int)]](Nil)) { case ((c,t), a) => { a ++ ( for {comb <- a; n <- 1 to t} yield (c, n) :: comb)} }
   ...
   */
  def subtract(x: Occurrences, y: Occurrences): Occurrences = y.foldLeft(x){ (nx, ey) => for (ex <- nx) yield (ex._1, if(ex._1 == ey._1) ex._2 - ey._2 else ex._2)}.filter(_._2 > 0)
